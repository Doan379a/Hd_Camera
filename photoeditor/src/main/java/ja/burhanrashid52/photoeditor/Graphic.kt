...
internal abstract class Graphic(
    ...
    private fun setupRemoveView(rootView: View) {
        ...
        val imgFlip = rootView.findViewById<ImageView>(R.id.imgPhotoEditorFlip)
        imgFlip?.setOnClickListener { flipImage() }
    }

    private fun flipImage() {
        val imgPhotoEditorImage = rootView.findViewById<ImageView>(R.id.imgPhotoEditorImage)
        if (imgPhotoEditorImage != null) {
            imgPhotoEditorImage.scaleX = -imgPhotoEditorImage.scaleX
        }
    }
    ...
    protected fun toggleSelection() {
        ...
        if (imgFlip != null) {
            imgFlip.visibility = View.VISIBLE
        }
    }
